...
BasePairMatrix::BasePairMatrix(std::string tsv_name)
{
    ...
    auto windows = readScanTSV(ifile);
...
void BasePairMatrix::getBestPairing(std::vector<BasePair>& pairs)
{
    /*
...
{   
    ...
    BasePairMatrix bpmatrix = BasePairMatrix(arg);
    bpmatrix.toCSV("matrix.csv");
    bpmatrix.print();
    std::vector<BasePair> pairs;
    bpmatrix.getBestPairing(pairs);
    std::ofstream ofile;
